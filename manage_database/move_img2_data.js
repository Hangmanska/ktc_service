        ...
        if (rows.length > 0)    
        {
            async.eachSeries(rows, function (row, next)
            {
              ...
              //  var sql = " COPY (select * from "+tb_name+" WHERE EXTRACT(YEAR FROM gps_datetime)!='2019') TO '/data/db_10011/"+tb_name+".csv' ";
               var sql = " COPY (select * from "+tb_name+"  WHERE EXTRACT(YEAR FROM idate) ='2021') TO '/data/db_10011_img_2021/"+tb_name+".csv' ";
...
function del_when_move_complete(tb_name,callback)
{
  ...
  sql+=" DELETE from "+tb_name+" WHERE EXTRACT(YEAR FROM idate)='2021' ";
 //  sql+=" DELETE from "+tb_name+" WHERE  gps_datetime <= '2020-09-30 23:59' ";
