...
function find_insert(blackbox_id, status_type, callback)
{
    ...
    switch (status_type)
    {
     ...
     // case '34': { sql = "SELECT id,idate(start_record) as start_record FROM history_status_havester  WHERE blackbox_id=" + utl.sqote(blackbox_id) + " AND status_type_start='33' AND start_record  IS NOT NULL AND end_record IS  NULL"; } break;
        ...
        case 77: { result.tb_name = 'history_status_havester', sql = "SELECT id,idate(start_record) as start_record FROM history_status_havester  WHERE blackbox_id=" + utl.sqote(blackbox_id) + " AND status_type_start=" + utl.sqote(status_type) + " AND start_record  IS NOT NULL AND end_record IS  NULL"; } break;//workout_zone
...
{
    if (table_name == 'history_status_truck')
    {
        var sql_insrt = squel.insert()
                       ...
                       .set('loading_harvest_vehicle_code', zone_data.harvester_name)
                       .toString();
...
setTimeout(function () {
    // test();
   var  start = {
       'blackbox_id': '8966181407448970000', 'r_time': '2015-06-14 08:24:35', 'r_status': '77' //r_status2
  ...
  var zone_data = { 'plot_code': '606527104', 'zone_id': '18' };

  check_history_status_havester(end, zone_data, function (x) {
