...
var db = require('iConnectdb_combine.js');
...
{
  var para =  req.body;
 // var para =  { id: 'ebd98549114e',start:'2020-06-23 00:00',stop:'2020-06-23 23:59'}
 ...
 /*
  {
        "receive_time": "2019-09-13 00:00:06",
        "rssi": -53,
        "battery": 2989,
        "temperature": 19.28,
        "humidity": 46,
 ...
 */
  ...
  db.get_rows(ipm, sql, function(rows)
  {
      if(rows.length>0)
      {
        ...
        var strMustache = '{{#.}}';
        strMustache += '{';
        strMustache += '"receive_time":"{{receive_time}}","id_server": "{{id_server}}" '
        strMustache +=' ,"rssi":{{data_sensor.rssi}},"battery":{{data_sensor.battery}},"temperature":{{data_sensor.temperature}},"humidity":{{data_sensor.humidity}} ';
        strMustache +=' ,"pressure":{{data_sensor.pressure}},"x":{{data_sensor.accelerationX}},"y":{{data_sensor.accelerationY}},"z":{{data_sensor.accelerationZ}}';
        strMustache += '}';
        strMustache += ',';
        strMustache += '{{/.}}';
...
{
    var sql = "SELECT id,tagid,data_sensor,idate(receive_time + interval '7' HOUR ) AS receive_time FROM tb_now ORDER BY id ";
    ipm.db.dbname = db_owner;
    db.get_rows(ipm, sql, function(rows)
    {
         res.send(rows);
...
{
    var sql = "SELECT id,tagid,data_sensor,idate(receive_time + interval '7' HOUR ) AS receive_time,id_server FROM tb_now ORDER BY id ";
    ipm.db.dbname = db_owner;
    db.get_rows(ipm, sql, function(rows)
    {
         res.send(beautify(rows, null, 2, 80));
    });
}

function xlist_all_sensor()
{
    var sql = "SELECT id,tagid,data_sensor,idate(receive_time + interval '7' HOUR ) as receive_time FROM tb_now ORDER BY id ";
    ipm.db.dbname = db_owner;
    db.get_rows(ipm, sql, function(rows)
     ...
     if(setting.length>0)
     {
      ...
      //  st.tempMax = 28
        if(data.temperature > st.tempMax)
        {
          ...
          if(st.snooze !='')
          {
              check_line_delay(id_sensor,st.snooze,is_true =>{
                if(is_true)
                {
...
/*
  Smart Monitor ⚠️ แจ้งเตือน ⚠️
❄️ [name]
[department],[floor]
🌡️ อุณหภูมิ[สูง/ต่ำ]กว่าที่กำหนด [หาค่าความต่าง] °C (จาก [tempMax หรือ tempMin] °C)
💦 ความชื้อ[สูง/ต่ำ]กว่าที่กำหนด [หาค่าความต่าง]% (จาก [humiMax หรือ humiMin]%)
⚠️ เลื่อนการแจ้งเตือน : http://www.cinnova.co.th/smartmonitor/snooze.php?id=[tagid]
*/
...
{
  var sf="";
  sf+=" Smart Monitor ⚠️ แจ้งเตือน ⚠️ \r\n";
  sf+=" ❄️ "+st_data.name+" "+st_data.department+", "+st_data.floor+ '\r\n';
  ...
  sf+="⚠️ เลื่อนการแจ้งเตือน : http://www.cinnova.co.th/smartmonitor/snooze.php?id="+st_data.tagid+" "+ '\r\n';
...
function check_line_delay(id_sensor,snooze_time,callback)
{
  var sql = "SELECT idate(receive_time + interval '7' HOUR ) as recive_time,idate(timer_checker_delay) as checker_time FROM tb_now WHERE tagid='"+id_sensor+"'  ";
  ipm.db.dbname = db_owner;
  db.get_rows(ipm, sql, function(rows)
  {
  //  rows[0].recive_time
...
var para =  { id: 'c8dc88061a9c',data: '{"dataFormat":3,"rssi":-67,"humidity":47,"temperature":19.47,"pressure":100490,"accelerationX":941,"accelerationY":429,"accelerationZ":-3,"battery":2899}' }
//check_for_send_LINE('c8dc88061a9c',para);
...
/*
check_line_delay('c8dc88061a9c','20',function(is_alert)
{
