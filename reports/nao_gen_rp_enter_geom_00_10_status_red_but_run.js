...
function main_process(para,callback)
{
    ...
    async.eachSeries(iar, function (row, next)
    {
        ...
        //  console.log(date_gen_report+' '+i);
         ...
         console.log('date_gen_report '+para.modem_id+' '+para.start_time+' '+  para.end_time);

         
         check_process_only_red(para,function(is_finish)
          {
              next();
          })
...
function check_process_only_red(para,callback)
{
    get_enter_geom(para,function(ress)
    {
        ...
        {
            ress =ress[0];
            para.start_time = ress.enter_time;
            para.end_time =  ress.leave_time;

            check_process(para,function(res_db)
            {
                ...
                temp.station = ress.geom_name;
    
                if(temp.station !=null)
                {
    
                    clear_data_now_by_id(para,function(xr)
                    {
...
function old_main_process(para,callback)
{
...
idate(MIN (gps_datetime)) AS start_engine,idate(MAX (gps_datetime)) AS off_engine 
 ...
 FROM ht_142181256623
 WHERE gps_datetime >='2019-09-01 00:00'
AND gps_datetime <='2019-09-01 23:59'
 AND status = '2' 
*/
  ...
  FROM x 
   ...
   var sql=''
    ...
    if(para.nao_group_sale !='X')
    {
        ...
        sql+=" ,fn_nao_prfix_zone_id2(geom_id::varchar) as id_zone,idate(enter_time) as enter_time,idate(leave_time) as leave_time  "
        sql+=" ,fn_nao_zone_name (fn_nao_prfix_zone_id2 (geom_id :: VARCHAR)) || '' || geom_name  as geom_name ";
...
function start(date_gen_report) 
{
   // var date_gen_report = '2019-05-02';db_name='db_10034'
    ...
    sql += " FROM master_config_vehicle WHERE db_name='db_10034' "
     sql += " AND modem_id='142181256548'   ORDER BY modem_id";
