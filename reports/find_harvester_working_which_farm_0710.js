...
function add_report( res, callback)
{
    ...
    var sql = " INSERT INTO harvester_report2(modem_id,farm_id,start_work,end_work,minute_working) VALUES " + result_val;
...
function start_manual()
{
    ...
    var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle  WHERE modem_id IN('1110011013') ORDER BY db_name DESC";
...
, 'start_time': '2021-12-06 00:00', 'end_time': '2021-12-06 23:59', 'date_gen_report': '2021-06-28', 'message': '' }
...
//start_manual(); '2021-12-22'

function test()
{

    var sql=" SELECT modem_id,harvester_name FROM harvester_register ;"

    var date_now = moment(moment().format('YYYY-MM-DD'))//moment('2015-09-02 00:00:00');
    var date_now_before = date_now.subtract(1, 'days');

    start_date = '2022-01-11';//date_now_before.format('YYYY-MM-DD')
    var start =  start_date+" 00:00";
    var end = start_date+" 23:59";
    ...
    db.get_rows(ipm, sql, function (res) 
    {
        if (res.length > 0) 
        {
            async.eachSeries(res, function (row, next) 
            {
                ...
                get_data(row.modem_id,start,end,function(result)
                {
...
schedule.scheduleJob(rule, function ()
{
   // console.log('Harvester Midnight This runs at 23:59 every day.');
    ...
    console.log('start find_harvester_working_which_farm_ ' + date_gen_report + ' timenow : ' +  moment().format('YYYY-MM-DD HH:mm:ss'));
  ...
  //  var end_time = date_gen_report+' 23:59';
    ...
    start_date = date_now_before.format('YYYY-MM-DD'); //'2022-01-11';//date_now_before.format('YYYY-MM-DD')
    var start =  start_date+" 00:00";
    var end = start_date+" 23:59";
