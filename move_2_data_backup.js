...
function del_record_forvacum(req, res)
{
    db_owner = 'db_10034'; //
        ...
        if (rows.length > 0)
        {
            async.eachSeries(rows, function (row, next)
            {
              ...
              //  var idx = row.id;
                //var sql = "COPY "+tb_name+" FROM '/var/lib/pgsql/9.5/data/"+tb_name+".csv';";
                //var sql = " COPY (select * from "+tb_name+" WHERE EXTRACT(YEAR FROM gps_datetime)!='2019') TO '/data/db_10011/"+tb_name+".csv' ";
             //   var sql = " DELETE FROM "+tb_name+" WHERE  ";
            //  var xtable =  schma_table(tb_name,idx);

        //      excute(db_owner,sql,tb_name,function(is_ok)
           //   {
                  console.log(' del '+tb_name);
...
function del_when_move_complete(db_owner,tb_name,callback)
{
  ...
  //  sql+=" DELETE from "+tb_name+" WHERE EXTRACT(YEAR FROM gps_datetime) !='"+year+"' ";
   sql+=" DELETE from "+tb_name+"  WHERE gps_datetime < '2020-06-01 00:00' ";
...
function move_to_backup_manual2()
{
     db_owner = 'db_10033'; //
     year = '2020'; //
     var db_backup='db_10033_month_1_4';
/*
    var sql="";
    sql+="  WITH res as( ";
    sql+="    SELECT row_number() OVER (ORDER BY table_name), table_schema,table_name";
  ...
  //  sql+="    --OR table_name LIKE 'img%'";
    ...
    //  var sql="";
   //  sql= "SELECT modem_id FROM realtime WHERE fleet_id='db_10033' AND   EXTRACT(YEAR FROM gps_datetime) < '2020' ";
    ...
    db2.get_rows(ipm2, sql, function (rows) 
    {
        if (rows.length > 0)
        {
            async.eachSeries(rows, function (row, next)
            {
//COPY 'db_10033' FROM '/root/ht_142190463254.csv' WITH CSV HEADER delimiter ',';
                debugger;
                var tb_name = utl.Trim( row.table_name)
              //  var idx = row.id;
                //var sql = "COPY "+tb_name+" FROM '/var/lib/pgsql/9.5/data/"+tb_name+".csv';";
...
/*
SELECT * FROM realtime WHERE fleet_id='db_10033'
AND   EXTRACT(YEAR FROM gps_datetime) < '2020';
*/

exports.move_to_backup = move_to_backup;
//move_to_backup();

//move_to_backup_manual2();
