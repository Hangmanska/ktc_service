...
{
    momentDate = moment(moment(momentDate).format('YYYY-MM-DD'))
   ...
   // console.log(row_count);
    if(row_count==0)
    {
        ...
        minus_datetime(start,function(res)
        {
            debugger;
           // console.log(xx);
            var sql =" SELECT oil_percent FROM "+tb_name+"  WHERE  gps_datetime >= "+utl.sqote(res.start)+" AND gps_datetime <="+utl.sqote(res.stop)+" ORDER BY gps_datetime DESC LIMIT 1";
            ipm.db.dbname = db_name;//db_owner;
            db.get_rows(ipm, sql, function (rows) 
            {
                debugger;
...
function is_found_onsetting_db(modem_id,callback)
{
   var sql="SELECT modem_id,db_name,vehiclename,is_calculate_fuel,is_fuel_invert,fuelempty,fuelfull,fueltank,db_name";
   sql+=" FROM master_config_vehicle WHERE is_calculate_fuel='1' AND modem_id="+utl.sqote(modem_id)+" AND db_name !='db_10003' ";
...
function main_cal_fuel_now(modem_id,start_date,callback)
{
    debugger;
  //  var modem_id ='1010004001';
  //  var start_date ='2017-04-26 00:00';
    
      var is_today =  isToday(start_date);
      if(is_today)
      {
            is_found_onsetting_db(modem_id,function(row)
            {
            debugger;
            ...
            var db_name = row[0].db_name;
            var modem_id = row[0].modem_id;
...
exports.main_cal_fuel_now = main_cal_fuel_now;
...
setTimeout(function () {
    ...
    var xdate =  '2017-04-09';//req.body.start; //
