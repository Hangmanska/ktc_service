...
debugger
   ...
   // var year_plant =  '64_65'; //req.body.year_plant;  //
...
sql += ",COALESCE(a.harvester_name,'-') as harvester_code ";
sql += ",COALESCE(get_colour_harvester(a.harvester_name),'#E5360f') as colour_code ";
...
sql += " FROM detail_farm_register_"+year_plant+" as a "
...
{ //
  ...
  //  var destination="16.80101,102.77213";
    var master_url ="https://maps.googleapis.com/maps/api/directions/json?origin="+source+"&destination="+destination+"&key=AIzaSyBdSsJqXVicO5C4ByIW_1AuK711JmAjOAo"

//https://maps.googleapis.com/maps/api/directions/json?origin="ยโสธร"&destination="ขอนแก่น"&key=AIzaSyBdSsJqXVicO5C4ByIW_1AuK711JmAjOAo"
 ...
 */
    ...
    console.log('1 report_driving_history '+JSON.stringify(req.body));

 var sql ='';var is_vt900 =false;

  if(utl.StartsWith(modem_id, '14'))
  {
...
function monitor_status_(req, res)
{
...
{
    var type_vehicle =   req.body.type_vehicle;// req.params.type_vehicle;//req.body.type_vehicle; 
    var modem_id = req.body.modem_id; //req.params.modem_id;
   // var year_plant =  '63_64'
  //  var db_owner = 'db_10011';
    ...
    if(type_vehicle=='0') // Harvester
    {
        ...
        sql +=",dblink_get_geofarm(htt_plotcode,'"+year_plant+"') as geojson_farm ";
       // sql +=",dblink_get_geofarm(dblink_get_farmid(lon::varchar,lat::varchar,'"+year_plant+"'),'"+year_plant+"') as geojson_farm ";
        sql +=",camera_url";
        sql +=" FROM realtime ";
        sql +=" WHERE htt_harvester_or_truck ='0'  ";
...
function find_near_me(req, res)
{
    var lon =   req.body.lon;// req.params.type_vehicle;//req.body.type_vehicle; 
    var lat = req.body.lat; //req.params.modem_id;
    var year_plant =  '63_64'
    var db_owner = 'db_10011';
      ...
      //  sql +=",dblink_get_geofarm(dblink_get_farmid(lon::varchar,lat::varchar,'"+year_plant+"'),'"+year_plant+"') as geojson_farm ";
        ...
        sql +="  ST_SetSRID( ST_Point("+lon+","+lat+"), 4326 )::geography ";
        sql +=" ,ST_SetSRID( ST_Point(lon,lat), 4326 )::geography, 3000) ";  
        sql +=" AND get_vehicle_name(modem_id) !='1234' ";
   
      //  console.log(db_owner);
...
function find_farm_near_me(req, res)
{
    var lon =   req.body.lon;// req.params.type_vehicle;//req.body.type_vehicle; 
    var lat = req.body.lat; //req.params.modem_id;
  ...
  //  var db_owner = 'db_10011';
        ...
        var sql="";
        sql +=" SELECT st_asgeojson(ST_force2D(geomx)) as geom  FROM detail_farm_register_"+year_plant+" ";
        sql +=" WHERE  ST_DWithin( ";
        sql +=" ST_SetSRID( ST_Point("+lon+","+lat+"), 4326 )::geography ";
        sql +=",ST_SetSRID( ST_Point(lon::FLOAT,lat::FLOAT), 4326 )::geography  ";
        sql +=" , 400)  ";
...
function report_harvester_working_on_farm(req, res)
{
    var start_date =req.body.start_date; 
    var end_date = req.body.end_date;
    var modem_id = req.body.modem_id; //req.params.modem_id;
    ...
    SELECT farm_id,get_namefarmer_6364(farm_id) as name_farmmer
    ,idate(start_work) as start_work,idate(end_work) as end_work,minute_working
    ,get_harvestername(modem_id) as harvester_work_on_farm
    ,get_harvestername_6364(farm_id) as harvester_on_plan
    ,fn_min_to_hrs(minute_working) as min_to_hours
     ,modem_id
    FROM harvester_report 
    WHERE minute_working >'1'
		AND modem_id='1110011013' 
	  AND start_work>='2021-01-16 08:05'
    AND end_work <='2021-01-21 08:49'
    ORDER BY modem_id 
    */

    var sql=" ";
    sql +="  SELECT farm_id,get_namefarmer_"+year_plant+"(farm_id) as name_farmmer ";
    sql +=" ,idate(start_work) as start_work,idate(end_work) as end_work,minute_working ";
    sql +=",get_harvestername(modem_id) as harvester_work_on_farm ";
    sql +=",get_harvestername_"+year_plant+"(farm_id) as harvester_on_plan ";
    sql +=",fn_min_to_hrs(minute_working) as min_to_hours ";
    sql +=" ,modem_id ";
    sql +=" FROM harvester_report2  ";
    sql +=" WHERE minute_working >='30' ";

	sql +=" AND start_work>='"+start_date+"' ";
    sql +=" AND end_work <='"+end_date+"' ";

    if (modem_id=='199999999969')
    {
      //  console.log(sql);
        nrows(sql,db_sugarcane,function  (rows){ res.send( rows); });

    }else{
        sql +=" AND modem_id='"+modem_id+"'  ";
        nrows(sql,db_sugarcane,function  (rows){ res.send( rows); });
    }
...
function get_data_realtime_ksp(req, res)
{
    ...
    sql += " AND to_char( r.gps_datetime::TIMESTAMP, 'YYYY-MM-DD') = to_char( now()::TIMESTAMP, 'YYYY-MM-DD') ";
...
function get_report_activity_farm(req, res)
{

    var year_plant = req.body.year_plant;   //'60_61'; // 
    var type_activity = req.body.type_activity;

    if(type_activity=='1') // Grow
   {
        var sql=" SELECT farmer_id,name_farmmer ";
        sql+=",FLOOR(ABS(st_area(geomx::geography)/1600)) as rai,eastimate ";
        sql+=" ,(SELECT name_colour_th FROM icolour_status WHERE icolour_code=id::varchar) as status_farm ";
        sql+=" ,farm_id,serveyor,manager,home,lat,lon ";
        sql+=" FROM detail_farm_register_"+year_plant+"  WHERE icolour_code IN ('3','4','5','7')  ";

        nrows(sql,db_sugarcane,function  (rows){ res.send( rows); });

   }

   else if(type_activity=='0') // NonGrow
   {
    var sql=" SELECT farmer_id,name_farmmer ";
    sql+=",FLOOR(ABS(st_area(geomx::geography)/1600)) as rai,eastimate ";
    sql+=" ,(SELECT name_colour_th FROM icolour_status WHERE icolour_code=id::varchar) as status_farm ";
    sql+=" ,farm_id,serveyor,manager,home,lat,lon ";
    sql+=" FROM detail_farm_register_"+year_plant+"  WHERE icolour_code IN ('6','8')  ";
...
function get_report_graph_harvester_cutting(req, res)
{
    var start_date =req.body.start_date; 

    var sql=" ";
    sql +="  SELECT r.modem_id,total_min_open_cutting ";
    sql +="  ,fn_min_to_hrs(total_min_open_cutting) as hh_mm ";
    sql +="  ,get_vehicle_name(r.modem_id) as vehicle_name  ";
    sql +="   FROM realtime as r,report_harvester_open_cutting as rp  ";
    sql +="   WHERE htt_harvester_or_truck ='0'   ";
    sql +="  AND r.modem_id=rp.modem_id  ";
    sql +="  AND rp.gps_datetime='"+start_date+"' ";
...
exports.find_near_me = find_near_me;
exports.find_farm_near_me  = find_farm_near_me;
exports.report_harvester_working_on_farm = report_harvester_working_on_farm;

exports.get_data_realtime_ksp = get_data_realtime_ksp;
exports.get_report_activity_farm = get_report_activity_farm;
exports.get_report_graph_harvester_cutting = get_report_graph_harvester_cutting;
...
var current_year ="64_65"

var tb_current_year="detail_farm_register_64_65";


  var sql =  " SELECT DISTINCT farm_id ";
  sql += ",st_y( st_centroid(geomx)) as lat_farm,st_x( st_centroid(geomx)) as lon_farm";
  sql +=  ",'17.072361' as lat,'102.923586' as lon "; //kmp
  sql +=  "FROM "+tb_current_year;
//  sql +=  " WHERE substr(qt, 0,3) NOT IN ('27','28') ";
  sql +=  " WHERE geomx IS NOT NULL AND distance_real_road IS  NULL"
