...
var status_engine_work = '3';//ก้ามแดงขึ้น + เครื่องยนต์ทำงาน
...
function has_status_trip(data, callback)
{
    ...
    .Where(function (x) { return x.input_status == '3' })
    .Count()
    callback(x_res);
    return;
}
...
function get_data(para,callback)
{
  //  console.log('get_data '+i);
    ...
    sql +=",mileage,modem_id"
    check_case_status(para.modem_id,function(result)
    {
        if(result=='1') // VCEW1458
        {
            ...
            sql += " ELSE status END as input_status ";
        }
        else
        {
...
function start(date_gen_report) 
{
    //AND modem_id='1010005002' 
      ...
      //  var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE  modem_id like '%14218%'   ORDER BY db_name";
        ...
        //WHERE db_name='db_10034' WHERE db_name='db_10034' WHERE db_name='db_10034'  AND modem_id='143200384940' AND modem_id='143190871335' 
        var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10036'    ORDER BY db_name DESC";
        console.log('start genreport ' + date_gen_report + ' timenow : ' + irp.timenow());
    
        //var date_gen_report = '2016-06-25'; AND modem_id='143200384772'
        ipm.db.dbname = db_config;
