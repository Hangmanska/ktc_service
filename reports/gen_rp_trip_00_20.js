...
/*
  SELECT idate(gps_datetime) as gps_datetime,mileage,speed,status,message_id 
  ,lon||','||lat as lonlat,tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en 
  FROM ht_1110011069 
   WHERE gps_datetime >='2019-11-11 11:00'
  AND gps_datetime <='2019-11-11 17:30'
  AND message_id in('11','193')  ORDER BY gps_datetime ASC 
*/
...
function start(date_gen_report) 
{
   ...
   var sql = "SELECT modem_id,db_name,speedmax,track_every,oil_level  FROM master_config_vehicle WHERE modem_id not like '%1421%' AND db_name !='db_10003' ";
   sql += " AND db_name !='db_10016' AND db_name !='db_10039' AND to_char(time_inserver, 'YYYY-MM-DD')=to_char(now(), 'YYYY-MM-DD')  ORDER BY modem_id  ";
  // var sql = "SELECT modem_id,db_name,speedmax,track_every,oil_level  FROM master_config_vehicle WHERE modem_id= '1110011069' ";
