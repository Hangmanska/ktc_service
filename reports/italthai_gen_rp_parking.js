...
function start(date_gen_report)
{
    ...
    //  var sql = "SELECT modem_id,db_name,'"+date_process+"' as date_process  FROM master_config_vehicle WHERE db_name='db_10036' ";
        ...
        var sql = "SELECT modem_id,db_name,speedmax,track_every, get_parkingstop(modem_id) as min_park    FROM master_config_vehicle WHERE db_name='db_10036'  ORDER BY modem_id";
        ...
        db.get_rows(ipm, sql, function (res) {
            if (res.length > 0) {
                async.eachSeries(res, function (row, next) {
                    debugger;
                    // console.log(row);
                    var para = { 'id': 1, 'db_name': row.db_name, 'modem_id': row.modem_id,'min_park':row.min_park , 'speed_max': row.speedmax, 'track_every': row.track_every, 'start_time': date_gen_report + ' 00:00', 'end_time': date_gen_report + ' 23:59', 'date_gen_report': date_gen_report, 'message': '' }
...
function start_manual(date_gen_report,callback)
{
   // var date_gen_report = '2020-10-14';  
    //var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10034'  ORDER BY modem_id";  AND modem_id='143190871335'
    var sql = "SELECT modem_id,db_name,speedmax,track_every, get_parkingstop(modem_id) as min_park   FROM master_config_vehicle WHERE db_name='db_10036'  ORDER BY modem_id";
...
var datetimer_start =1;
function doit()
{
    var start_date = '2021-03-'+datetimer_start;
    start_manual(start_date,function(is_fin)
    {
        if(is_fin)
        {
            if(datetimer_start < 13)
            {
