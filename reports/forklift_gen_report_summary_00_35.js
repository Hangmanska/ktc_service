...
ORDER BY gps_datetime ASC 
   ...
   SELECT r.modem_id,'2021-10-07' as date_process 
    ...
    AND sv.modem_id=r.modem_id 
		AND r.modem_id NOT IN (
    SELECT modem_id FROM  rp_forklift_summary WHERE to_char(date_process,'YYYY-MM-DD')='2021-10-01')
...
function get_all_vehicle(date_process)
{
   ...
   sql += " AND sv.modem_id=r.modem_id 	AND r.modem_id NOT IN ( "
    sql += " SELECT modem_id FROM  rp_forklift_summary WHERE to_char(date_process,'YYYY-MM-DD')='"+date_process+"') "
...
rule.hour = 00//23;
rule.minute = 35//59;
//https://stackoverflow.com/questions/34735908/scheduling-file-every-1-hour-using-nodejs
//#region
