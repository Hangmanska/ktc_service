...
var dblink_master = 'dbname=dbu_master54 port=5432';
var dblink_dbu101279 = 'dbname=dbu_101279 port=5432';

var db_htt_master = {
	"host": "203.151.4.8",//"210.4.143.54",
	"port": "5432",
	"user": "rdteam",//"postgres",
	"pws": "rd@2015",//"monitormobile@01",
	"dbname": "dbu_master54"
};

var pg_master_HTT = new db.im2(db_htt_master);
			...
			async.eachSeries(array, function (row, next) {
				...
				fs.writeFile(new_path, stringCsv, function (err) {
					...
					db.excute(pg_config_HTT, query, function (response) {
						...
						if (response == 'oK') {
							var query_truncate = "TRUNCATE TABLE htt1 RESTART IDENTITY";
							db.excute(pg_config_HTT, query_truncate, function (responseidt) {
...
function update_htt1(req, res) {
	...
	query += " WHERE  f_id = " + utl.sqote(f_id) + " AND qt = " + utl.sqote(qt);
...
function delete_htt1(req, res) {
	var f_id = req.body.f_id;
	var qt = req.body.qt;
	
	var query = "DELETE FROM htt1";
	query += " WHERE  f_id = " + utl.sqote(f_id) + " AND qt = " + utl.sqote(qt);
...
function csv_htt1(req, res) {
	var query = "SELECT ST_AsGeoJSON(wkt_geom), f_id, qt, area, gis_id, cane_type, sub_zone, "; 
	query += "zone_id, product_to, name, cane_width, car_name, geo_x, geo_y FROM htt1";
	db.get_rows(pg_config_HTT, query, function (rows) {
		if (rows.length > 0) {

			var fin = new LINQ(rows).Select(function (x) {
				return {
					wkt_geom: formatter(x.st_asgeojson),
					f_id: x.f_id,
					qt: x.qt,
					are: x.area,
					gis_id: x.gis_id,
					cane_type: x.cane_type,
					sub_zone: x.sub_zone,
					zone_id: x.zone_id,
					product_to: x.product_to,
					name: x.name,
					cane_width: x.cane_width,
					car_name: x.car_name,
					geo_x: x.geo_x,
					geo_y: x.geo_y
				}
			...
			/*var strMustache = '{{#.}}';
			strMustache += '{';
			strMustache += ' wkt_geom: {{st_asgeojson}},';
			strMustache += ' F_ID: "{{f_id}}",';
			strMustache += ' QT: "{{qt}}",';
			strMustache += ' AREA: "{{area}}",';
			strMustache += ' GID_ID: "{{gis_id}}",';
			strMustache += ' CANE_TYPE: "{{cane_type}}",';
			strMustache += ' SUB_ZONE: "{{sub_zone}}",';
			strMustache += ' ZONE_ID: "{{zone_id}}",';
			strMustache += ' PRODUCT_TO: "{{product_to}}",';
			strMustache += ' NAME: "{{name}}",';
			strMustache += ' CANE_WIDTH: "{{cane_width}}",';
			strMustache += ' CAR_NAME: "{{car_name}}",';
			strMustache += ' geo_x: "{{geo_x}}",';
			strMustache += ' geo_y: "{{geo_y}}"';
			strMustache += '}';
			strMustache += ',';
			strMustache += '{{/.}}';
			
			var result = mustache.render(strMustache, fin);
			result = utl.iRmend(result);*/
...
function formatter(str) {
	var sumStr = '',
		...
		arr = arrStr.coordinates[0];
	//for (var i in arr) sumStr += String(arr[i]).replace(',', ' ') + ', ';
	for (var i in arr) sumStr += iutm.Wgs2Utm(arr[i][0], arr[i][1]) + ", ";
	var final = startStr + utl.iRmend(utl.Trim(sumStr)) + endStr;
	return final;
}
...
exports.upload_HTT2 = upload_HTT2;
exports.update_htt1 = update_htt1;
exports.delete_htt1 = delete_htt1;
exports.csv_htt1 = csv_htt1;
...
function main_query_harvester(irule,callback) {
	var query_a = ''; var query_b = '';
 
	// var irule = { "rule": "truckgroup", "user_id": "UEVO-00603", "truckgroup_id": "'TG-00240','TG-00235','TG-00239'" }
	irule = JSON.parse(irule);
		...
		case 'all':
			{
			 //   query_a += " SELECT count(*) as harvester_total FROM htt4";
				...
				query_b += " ,r_lon,r_lat,r_time,_r_datasend,tambol,amphur,province,htt4.name";
				query_b += " ,htt_status_useability,htt4.name,htt4.bp,ms_havester.telephone";
				query_b += ",ms_havester.telephone,htt_plotcode,htt_name_farmer,htt_phone_farmer,htt_workout_zone,icount.harvester_total";
				// aob editted
				//query_b += " , r_status, lower(htt_status_operation) as htt_status_operation";
				query_b += " ,CASE WHEN substring(CAST(r_time AS VARCHAR), 0, 11) <> substring(CAST(now() AS VARCHAR), 0, 11) THEN 33";
				query_b += " ELSE r_status END AS r_status";
				query_b += " ,CASE WHEN substring(CAST(r_time AS VARCHAR), 0, 11) <> substring(CAST(now() AS VARCHAR), 0, 11) THEN 'stop'";
				query_b += " ELSE lower(htt_status_operation) END AS htt_status_operation";
		...
		case 'truckgroup':
			{
			  ...
			  //  query_a += " SELECT count(*) as harvester_total FROM htt4";
				query_b += " SELECT";
				query_b += " DISTINCT rec_now.blackbox_id ";
				query_b += " ,ms_havester.harvester_number";
				query_b += " ,r_lon,r_lat,r_time,_r_datasend,tambol,amphur,province,htt4.name";
				query_b += " ,htt_status_useability,htt4.name,htt4.bp,ms_havester.telephone";
				query_b += ",ms_havester.telephone,htt_plotcode,htt_name_farmer,htt_phone_farmer,htt_workout_zone,icount.harvester_total";
				...
				query_b += " ,dblink(" + utl.sqote(dblink_HTT) + ", 'SELECT havest_vehicle_code,havest_telephone FROM master_havester') as ms_havester (harvester_number varchar(50),telephone varchar(20))";
				query_b += " ,dblink(" + utl.sqote(dblink_HTT) + ", 'SELECT COUNT(bp) FROM htt4') as icount (harvester_total int)";
				query_b += " ,dblink(" + utl.sqote(dblink_master) + ", 'SELECT blackbox_id,truckgroup_id FROM truck_group') as rule_truck (blackbox_id varchar(50),truckgroup_id VARCHAR(100))";
				query_b += " WHERE blackbox.blackbox_id = rec_now.blackbox_id";
				query_b += " AND truck.truck_id = blackbox.truck_id";
				query_b += " AND htt4.blackbox_id=rec_now.blackbox_id";
				query_b += " AND ms_havester.harvester_number = htt4.harvester_number";
				query_b += " AND rule_truck.blackbox_id = blackbox.blackbox_id "
				query_b += " AND rule_truck.truckgroup_id in (" + irule.truckgroup_id + ')'
			} break;
	}
  //  var res ={'a':query_a,'b':query_b };
	callback(query_b);
	return;
...
function tracking_havester(req, res) {
	//priority
	debugger;
	var irule = req.params.priority; //
	//var irule = '{"rule":"all","user_id":"UEVO-00603","truckgroup_id":""}'
   // irule = JSON.parse(irule);
   // var irule = { "rule": "all", "user_id": "UEVO-00603", "truckgroup_id": "'TG-00240','TG-00235','TG-00239'" }
	var object = { "vehicle_all": 0, "vehicle_tracking": [] };
  //  var finish_a = false; var finish_b = false;

	main_query_harvester(irule, function (query_b) {
		  ...
		  //  finish_b = true;
			if (rows.length > 0) {
			   // finish_a = true;
				...
				object.vehicle_all = rows.length
				res.send(object);
			}
		   // final();
		});
...
function isDay_performance(req, res) {
	...
	db.get_rows(pg_config_HTT, query, function (rows) {
		if (rows.length > 0) {
			...
			object.stop_sum = new LINQ(object.performance).Select(function (x) { return parseInt(x.stop) || 0 }).Sum();
			object.idle_sum = new LINQ(object.performance).Select(function (x) { return parseInt(x.idle) || 0 }).Sum();
			object.run_sum = new LINQ(object.performance).Select(function (x) { return parseInt(x.run) || 0 }).Sum();
			object.cut_load_sum = new LINQ(object.performance).Select(function (x) { return parseInt(x.cut_load) || 0 }).Sum();
			debugger;
		}
...
function isDay_outsize(req, res) { 
	var query = " SELECT idate(date_report) as date_report";//,total_harvester";
	query += ", (SELECT COUNT(DISTINCT get_harvester(blackbox_id)) FROM history_status_havester WHERE status_type_start = '77' ";
	query += " AND get_ymd(start_record) >= " + utl.sqote(req.params.start_time) + " AND get_ymd(start_record) <= " + utl.sqote(req.params.end_time);
	query += " AND length(plot_code_start) > 1 ";
	query += " AND datediff('hour', start_record, end_record) IS NOT NULL AND get_harvester(blackbox_id) IS NOT NULL ) AS total_harvester ";
	query += " ,total_workout_zone, time_workout_zone";
	query += " FROM harvester_workout_zone_report";
	query += " WHERE date_report >= " + utl.sqote(req.params.start_time);
	query += " AND date_report <= " + utl.sqote(req.params.end_time);
...
function isAll_outsize(req, res) { 
	var query = " SELECT idate(h.date_report) as date_report";//,total_harvester";
	query += " ,(SELECT COUNT(DISTINCT get_harvester(blackbox_id)) FROM history_status_havester WHERE status_type_start = '77' ";
	query += " AND get_ymd(start_record) = get_ymd(h.date_report) ";
	query += " AND length(plot_code_start) > 1 ";
	query += " AND datediff('hour', start_record, end_record) IS NOT NULL AND get_harvester(blackbox_id) IS NOT NULL ) AS total_harvester ";
	query += " ,h.total_workout_zone, h.time_workout_zone";
	query += " FROM harvester_workout_zone_report AS h";
...
function main_query_truck(irule,callback) {
	var query_a = ''; var query_b = '';
		...
		case 'all':
			{
				...
				query_b += " ,rec_now.blackbox_id,_r_datasend"
				query_b += " ,r_lon,r_lat,tambol,amphur,province";
				query_b += " ,htt_status_useability";
				...
				// AOB EDIT
				//query_b += ", lower(htt_status_truck) as htt_status_truck, r_status, lower(htt_status_operation) as htt_status_operation";
				query_b += " ,CASE WHEN substring(CAST(r_time AS VARCHAR), 0, 11) <> substring(CAST(now() AS VARCHAR), 0, 11) THEN 'empty'";
				query_b += " ELSE lower(htt_status_truck) END AS htt_status_truck";
				query_b += " ,CASE WHEN substring(CAST(r_time AS VARCHAR), 0, 11) <> substring(CAST(now() AS VARCHAR), 0, 11) THEN 33";
				query_b += " ELSE r_status END AS r_status";
				query_b += " ,CASE WHEN substring(CAST(r_time AS VARCHAR), 0, 11) <> substring(CAST(now() AS VARCHAR), 0, 11) THEN 'stop'";
				query_b += " ELSE lower(htt_status_operation) END AS htt_status_operation";
...
function tracking_truck(req, res) {
	...
	main_query_truck(irule, function (query_b) {
...
function working_truck(req, res) {
	...
	/*var query_b = " SELECT htt7.truck_code";
	query_b += " ,truck.truck_name";
...
function inDay_waiting(req, res) {
	...
	db.get_rows(pg_config_HTT, query, function (rows) {
		if (rows.length > 0) {
			object.waiting = rows;
			object.infarm = new LINQ(rows).Select(function (x) { return parseInt(x.waiting_infarm) || 0 }).Sum();
			object.parkOutsize = new LINQ(rows).Select(function (x) { return parseInt(x.waiting_parkoutside) || 0 }).Sum();
			object.parkPrepare = new LINQ(rows).Select(function (x) { return parseInt(x.waiting_prepare) || 0 }).Sum();
			object.parkInsize = new LINQ(rows).Select(function (x) { return parseInt(x.waiting_inside) || 0 }).Sum();
		}
		res.send(object);
	});
}
	...
	db.excute(pg_config_HTT, query, function (response) {
		if (response == 'oK') {
			...
			db.get_rows(pg_config_HTT, sql, function (rows)
			{
				...
				console.log('insert_vehicle_lost ' + req.body.vehicle_name + '-' + req.body.vehicle_type + '-' + req.body.start_lost + '-' + req.body.end_lost + '-' + rows[0].id)
				iOut.start_out_of_service(req.body.vehicle_name, req.body.vehicle_type, req.body.start_lost, req.body.end_lost, rows[0].id, function (value) {
					res.send('Insert-Complete');
				});
			});
...
function set_report(rows, vehicle, callback) {
	var truck_lose = new LINQ(rows).Where(function (x) { return x.status_type_start == 88 }).ToArray();
	var truck_stop = new LINQ(rows).Where(function (x) { return x.status_type_start == 33 }).ToArray();
	var truck_idle = new LINQ(rows).Where(function (x) { return x.status_type_start == 31 }).ToArray();
	var truck_run = new LINQ(rows).Where(function (x) { return x.status_type_start == 30 }).ToArray();
	var truck_cutcane = new LINQ(rows).Where(function (x) { return x.status_type_start == 10 }).ToArray();
	var truck_putcane = new LINQ(rows).Where(function (x) { return x.status_type_start == 11 }).ToArray();
	...
	var truck_detail = {
		...
		"vehicle_idel": set_sumary(truck_idle),
		"vehicle_run": set_sumary(truck_run),
		"vehicle_cutcane": set_sumary(truck_cutcane),
		"vehicle_putcane": set_sumary(truck_putcane),
	}
	...
	var data_return = {
		...
		"truck_idle": truck_idle,
		"truck_run": truck_run,
		"truck_cutcane": truck_cutcane,
		"truck_putcane": truck_putcane
	};
...
function reletion_tracking(req, res) {
	...
	switch (req.params.sql_colum) {
		case 'truck_vehicle_code': {
			...
			sql_linq += " htt_harvester_name as harvester_name,";
			sql_linq += " get_owner_truck(htt_harvester_name) as name";
			sql_linq += " FROM rec_now as rc";
		...
		case 'havest_vehicle_code': {
			...
			sql_linq += " htt_truck_name as truck_name,";
	...
	query_b += " SELECT";
	query_b += " rc.blackbox_id,";
	query_b += " rc.r_lon,";
	query_b += " rc.r_lat,";
	query_b += " rc.r_time,";
	query_b += " rc._r_datasend,";
	query_b += " rc.tambol,";
	query_b += " rc.amphur,";
	query_b += " rc.province,";
	query_b += " rc.r_status,";
...
function bound_relation_area(req, res) {
	//var query = " SELECT wkt_geom,f_id FROM htt1";
	//query += " WHERE ST_Contains(ST_MakeEnvelope(" + utl.sqote() + "," + utl.sqote() + "," + utl.sqote() + "," + utl.sqote() + ", 4326), wkt_geom)";
...
function list_master_harvester(req, res) {
   // var query = "SELECT * FROM master_havester";
...
function decode($string, $key) {
	...
	for (var $i = 0; $i < $strLen; $i += 2) {
		var $ordStr = php.hexdec(php.base_convert(php.strrev(php.substr($string, $i, 2)), 36, 16));
		if ($j == $keyLen) { $j = 0; }
		var $ordKey = php.ord(php.substr($key, $j, 1));
		$j++;
		$hash += php.chr($ordStr - $ordKey);
	}
	return $hash;
}
...
function encode($string, $key) {
	...
	for (var $i = 0; $i < $strLen; $i++) {
		var $ordStr = php.ord(php.substr($string, $i, 1));
		if ($j == $keyLen) { $j = 0; }
		var $ordKey = php.ord(php.substr($key, $j, 1));
		$j++;
		$hash += php.strrev(php.base_convert(php.dechex($ordStr + $ordKey), 16, 36));
	}
	return $hash;
}


function get_role_see_vehicle(user_id,callback) {
   //debugger;
   var query = " SELECT ut.*,w.canread,w.priority_id FROM work_config as w left join truck_group_user as ut on ut.user_id = w.user_id ";
  query += " WHERE w.user_id=" + utl.sqote(user_id) + " and (w.priority_id='P01010' or w.priority_id='P01002') AND w.canread !='0' ORDER BY priority_id ASC ";
  var xres = {'rule':null,'user_id':' ','truckgroup_id':''};

  db.get_rows(pg_master_HTT, query, function (rows)
  {
		//debugger;
		if (rows.length > 0)
		{
			...
			{
				if (row['priority_id'] == 'P01002' && row['canread'] == '1')
				{
					xres.rule = 'all';
					xres.user_id = user_id;
				}
				else
				{
						if (row['priority_id'] == 'P01010' && row['canread'] == '1') {
							if (row['truckgroup_id'] == '' || row['truckgroup_id']==null) {
								xres.rule = 'all';
								xres.user_id = user_id;
							} else {
								xres.rule = 'truckgroup'
								xres.truckgroup_id += utl.sqote(row['truckgroup_id']) + ',';
								xres.user_id = user_id;
							}
						}
			...
			{
				var truck_group = utl.Trim(xres.truckgroup_id);

				xres.truckgroup_id = utl.iRmend(truck_group);
				callback(xres);
				return;
				//if (utcp.EndsWith(truck_group, ','))
				//{
				//  console.log('finish ' + truck_group);
				//}
			  //  JSON.stringify(xres)
			});
...
function get_login(req, res) {
  ...
  var xdata = { 'user': req.params.name, 'pws': req.params.pass };
  //  var xdata = { 'user': 'UEVO-00603', 'pws': 'dtcgps' };
	var decode_pws = utl.Trim(encode(xdata.pws, "DTCGPS"));

	var query = "SELECT user_id,user_login,user_password,user_name,user_active FROM users WHERE user_login=" + utl.sqote(xdata.user) + ' AND user_password=' + utl.sqote(decode_pws);

	//var query = "SELECT user_id,user_login,user_password,user_name,user_active FROM users WHERE user_login=" + utl.sqote(xdata.user);
	db.get_rows(pg_master_HTT, query, function (rows) {
		//debugger;
		if (rows.length > 0)
		{
			/*
			var is_equal =utl.Trim(rows[0].user_password);
			if (decode_pws == is_equal)
			{
			   // get_role_see_vehicle(rows[0].user_id, function () { });
				res.send('success-login');
			}
			*/
			get_role_see_vehicle(rows[0].user_id, function (res_rule) {
				
				var result = { 'status_login': 'success-login','status_rule':res_rule}
				res.send(result);
			});
		   
		   
		} else {
			var result = { 'status_login': 'notfound-login', 'status_rule': res_rule }
			res.send(result);
...
function set_geJson(data, callback) {
	var geoArr = {
	...
	async.eachSeries(data, function (row, next) {
		var strJson = {
			...
			"south": row.south,
			"qt": row.qt,
			"properties": { "name" : row.gis_id },
			"geometry": JSON.parse(row.st_asgeojson)
		};
		geoArr.features.push(strJson);
		next();
	}, function () {
		callback(geoArr);
		return;
...
setTimeout(function () {
  //get_login(' ',' ');
   // get_role_see_vehicle('UEVO-00608');
	tracking_havester(' ',' ');
}, 1000);
*/