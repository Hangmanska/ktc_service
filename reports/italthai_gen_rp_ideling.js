...
function get_data(para,callback) 
{
    try
    {
        ...
        quer_filter_is_driving = " SELECT COUNT(speed) FROM ht_" + para.modem_id + " 	WHERE gps_datetime::TIMESTAMP >=" + utl.sqote(para.start_time) + "	AND gps_datetime::TIMESTAMP <=" + utl.sqote(para.end_time) + " AND status ='3' ";

        var sql = ' ';
        sql += " SELECT idate(gps_datetime::TIMESTAMP) as gps_datetime,lon||','||lat as lonlat,tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en FROM ht_" + para.modem_id + " ";
...
function start(date_gen_report) 
{
  ...
  // var date_gen_report = '2016-09-24';
    //WHERE modem_id='143200384772'
    var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10036' WHERE modem_id='143200384772'  ORDER BY modem_id";
    ipm.db.dbname = db_config;
...
function start_manual(date_gen_report,callback) {
  ...
  // var date_gen_report = '2016-09-24';
    //WHERE modem_id='143200384772'
    var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10036' AND modem_id='143200384772'  ORDER BY modem_id";
    ipm.db.dbname = db_config;
    db.get_rows(ipm, sql, function (res)
    {
        if (res.length > 0)
