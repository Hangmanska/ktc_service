...
function get_vehicle_info_forklift(req, res)
{
    ...
    SELECT DISTINCT mcv.modem_id,mcv.sim,mcv.vehiclename
    ,mcv.carlicence,mcv.speedmax,mcv.idlestop,mcv.chassesno 
    ...
    ,calibrate_working_hour as calibrate_work_time
    ,calibrate_battery
    ,group_zone
    ...
    AND mcv.db_name=sv.fleetid 
    AND sv.modem_id= mcv.modem_id 
    ORDER BY mcv.modem_id ASC
    */
    //#endregion
...
function chart_percent_battery_forklift(req, res) 
{
    ...
    var tb_name = "ht_" + modem_id;
                ...
                sql += " ,coalesce(analog_input1,'0') as voltage_battery";
                sql += ",tambol||':'||amphur||':'||province as tloc ";
                sql += ",etambol||':'||eamphur||':'||eprovince as eloc ";
                sql += ",status,get_forklift_minbat('"+modem_id+"') as min_bat,get_forklift_maxbat('"+modem_id+"') as max_bat ";
                sql += " FROM " + tb_name;
                sql += " WHERE  gps_datetime >= " + utl.sqote(start);
                sql += " AND gps_datetime <=" + utl.sqote(stop);
                sql += " ORDER BY gps_datetime ASC ";
                ...
                has_table(modem_id,db_name,function(resx)
                {
                    ...
                    //      console.log('chart_oil_percent2 db_name ='+db_name+' modem_id='+modem_id+' leangt='+ rows.length)
                            ...
                            //  strMustache += '"time":{{gps_datetime}},"speed":{{speed}},"loc":{{tloc}}';
                                ...
                                var strMustache2 = '{{#.}}';
                                strMustache2 += '{{voltage_battery}}';
                                strMustache2 += ',';
                                strMustache2 += '{{/.}}';
        
                                var strMustache3 = '{{#.}}';
                                strMustache3 += '"{{tloc}} | {{voltage_battery}} เปอร์เซ็นต์แบตเตอรี่"';
                                strMustache3 += ',';
                                strMustache3 += '{{/.}}';
        
                                var strMustache4 = '{{#.}}';
                                strMustache4 += '"{{eloc}} | {{voltage_battery}} Percent_Battery"';
                                strMustache4 += ',';
                                strMustache4 += '{{/.}}';
                               ...
                               var result0 = '{ "min_bat":' +utl.sqote( rows[0].min_bat) + ',"max_bat":'+utl.sqote(rows[0].max_bat)+' }';
                                ...
                                res.send('['+result0+','+result1 +','+ result2+','+ result3+','+ result4+','+status+']');
                            }
                            else {
                                res.send([]);
                            }
...
/*
    ...
    */
    ...
    sql1 += " ,round(dblink_forklift_sum_mileage('db_10039',modem_id,idate(start_date),idate(end_date)),4) as distance ";
