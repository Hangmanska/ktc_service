...
function get_data(para,callback) 
{
    ...
    {
        var sql = ' ';
        sql += " SELECT input_status,idate(gps_datetime) as gps_datetime,lon||','||lat as lonlat,speed,mileage,tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en FROM ht_" + para.modem_id + " ";
        sql += " WHERE gps_datetime >=" + utl.sqote(para.start_time) + " AND gps_datetime <=" + utl.sqote(para.end_time) + " ";
        sql += " AND  satelites > 0 ORDER BY gps_datetime  "
...
function start()
{
    var date_gen_report = '2019-02-01';
    var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle   ORDER BY modem_id";
    ipm.db.dbname = db_config;
    db.get_rows(ipm, sql, function (res) {
        if (res.length > 0) {
