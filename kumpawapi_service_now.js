...
//sql += ",_getdetail_picture(farm_id) as details_picture ";
...
//,ประเภทอ้อย,พันธุ์อ้อย,แหล่งพันธุ์,วันที่ปลูก,ระยะปลูก,วิธีการปลูก,ผู้ปลูก,คาดการณ์ผลผลิต(ลำ/ไร่)
    ...
    db.get_rows(ipm, sql, function (rows) {
        if (rows.length > 0)
        {
            ...
            strMustache += ' ,"id_picture":"{{id_picture}}"';
            strMustache += '}';
            strMustache += ',';
            strMustache += '{{/.}}';
...
{ //2017-11-26 00:00 - 2017-11-27 23:59
 ...
 // sql +="  SELECT row_number() OVER (ORDER BY gps_datetime) as id ";
  ...
  sql +=" AND satelites >'0' AND message_id !='2' AND message_id !='11' ";
...
{
    var type_vehicle =  req.params.type_vehicle;//req.body.type_vehicle; 
    var modem_id =  req.params.modem_id;
...
function cal_real_road_distance()
{
  ...
  sql += ",st_y( st_centroid(geomx)) as lat_farm,st_x( st_centroid(geomx)) as lon_farm";
  sql +=  ",'17.10047' as lat,'103.024124' as lon ";
  sql +=  "FROM detail_farm_register_62_63 WHERE farm_zone='KMP' AND geomx IS NOT NULL ";
 // sql +=  " WHERE substr(qt, 0,3) NOT IN ('27','28') ";
                ...
                var destination= row.lat_farm+','+row.lon_farm; //"16.80101,102.77213";
                    ...
                    // executes after one second, and blocks the thread
                        ...
                        get_distance_direction(source,destination,function(xresult)
                        {
                            ...
                            var sql = squel.update()
                            .table('detail_farm_register_62_63')
