...
function what_time_exit_factory(modem_id,start,end,id_key,db_name,callback)
{
  ...
  //  var end = date+" 23:59";
    ...
    db.get_rows(ipm, sql, function (res) 
    {
        ...
        else
        {
            ...
            set_report('000','2021-09-18 00:00',id_key,function(xres)
            {
                callback(true);
                return;
            })
...
function set_report(factory_id,gps_datetime,id_key,callback)
{
    ...
    var sql= " UPDATE truck_work_in_farm2 SET time_exit_factory='"+gps_datetime+"'  WHERE id='"+id_key+"' ";
...
function start()
{
    ...
    sql += " to_char(time_enter_factory, 'YYYY-MM-DD HH24:MI') as tk_start";
    sql += ",to_char(time_enter_factory, 'YYYY-MM-DD')||' 23:59' as tk_end";
    sql += ",b.modem_id as modem_id, b.id as id_key";
    sql += " FROM harvester_report2 as a,truck_work_in_farm2 as b  ";
    sql += " WHERE b.farm_id=a.farm_id  ";
    sql += " AND factory_id !='999' ";
    sql += " AND to_char(a.start_work, 'YYYY-MM-DD')=to_char(b.start_work, 'YYYY-MM-DD')  ";
        ...
        if (res.length > 0) 
        {
            async.eachSeries(res, function (row, next) 
            {
                ...
                has_table(row.modem_id,db_focus,function(result)
                {
                    if(result)
                    {
                        what_time_exit_factory(row.modem_id,row.tk_start,row.tk_end, row.id_key,"db_10011",function(xrds)
                        {
                            next();
                        });
                    }
                    ...
                    {
                        what_time_exit_factory(row.modem_id,row.tk_start,row.tk_end, row.id_key,"db_10023",function(xrds)
                        {
                            next();
                        });
                    }
