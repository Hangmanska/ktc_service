   ...
   Isauthenticate(req, res, function () {
        ...
        else
        {
          // console.log(object);
           ...
           sql += "     WITH result as( ";
           sql += " SELECT row_number() OVER (ORDER BY r.modem_id) as id,r.modem_id ";
           sql += " ,get_vehiclename(r.modem_id) as vehicle_name,camera_url  ";  
           sql += " FROM	realtime as r, setup_vehicle as sv ";
           sql += "     WHERE	r.modem_id=sv.modem_id ";
           sql += "     AND sv.fleetcode=get_fleetid("+ utl.sqote(object.fleetname) +") ";
           sql += " )  ";
...
,(SELECT COUNT(colour) FROM noti_details WHERE is_read='0' AND fleetid='db_10001') as total_unread
FROM noti_details 
 WHERE is_read='0' AND fleetid='db_10001'
      ...
      // var fleet_id = 'db_10001'
       var sql1 = ""; 
        sql1 += " SELECT date_event,message_th,message_en,colour,lon,lat  ";
         sql1 += " ,(SELECT COUNT(colour) FROM noti_details WHERE is_read='0' AND fleetid="+ utl.sqote(fleet_id)+") as total_unread";
        sql1 += " FROM noti_details WHERE is_read='0' AND fleetid=" + utl.sqote(fleet_id);
...
var sql = "";
    ...
    sql += ",driver_no,CASE WHEN driver_sex='1' THEN 'ชาย/Male'  ELSE 'หญิง/Female'  END as driver_sex ";
