...
//+++++++++++ regen_report_working_history ++++++++++
function regen_report_working_history(req, res)
{
    var db_name = req.body.fleetid; //'db_10034';
    var modem_id = req.body.modemid; //'1010003020'; //
    var year_month = req.body.year_month; // '2019-05-20 00:00';//
    ...
    regen_process(para,function(is_complete)
        {
         //   console.log('is_complete '+is_complete);
            ...
            if(is_complete)
            {
                ...
                sql += " WHERE modem_id="+utl.sqote(modem_id);
                sql += " AND start_date >="+ utl.sqote(start_date);
                sql += " AND start_date <="+ utl.sqote(stop_date);
                sql += " ORDER BY start_date ASC ";
...
*/
    ...
    if(right !=undefined)
    {
        ...
        {
            sql+= " WHERE zone_id::text IN (SELECT split_to_rows('"+nao_group_sale+"',',')::text ) ";
     ...
     //   console.log(sql);
        ...
        db.get_rows(ipm, sql, function (rows) 
        {
            if (rows.length > 0)
            {
                ...
                strMustache += ' ,"admin": "{{admin}}","station_name": "{{zone_id}} {{station_name}}","zone_id": "{{zone_id}}"';
...
LIMIT 10
    */
    ...
    sql+=" ,station_name";
    sql+=" ,fn_nao_zone_name(zone_id::varchar) as zone_name";
    ...
    db.get_rows(ipm, sql, function (rows) 
    {
           ...
           async.eachSeries(rows, function (row, next)
       {
           ...
           xres.station_name = row.zone_name+' '+row.station_name;
...
function regen_process(para,callback)
{
    svnao.clear_data_now_by_id(para,function(xres)
    {
       // console.log('clear_data_now_by_id '+xres);
...
exports.regen_report_working_history = regen_report_working_history;
//#region Test
...
//get_report_working('','')
/*
var para = { 'id': 1, 'db_name': 'db_10034', 'modem_id': '142181155716', 'start_time':'2019-08-22 00:00' , 'end_time': '2019-08-22 23:59' , 'message': '' }
is_process_now(para,function(x){
    console.log(x);
})
*/
//#endregion
