...
var status_engine_work = '2';//ก้ามแดงขึ้น + เครื่องยนต์ทำงาน
...
function get_data(para,callback)
{
  //  console.log('get_data '+i);
    ...
    sql += ",idate(gps_datetime::TIMESTAMP) as gps_datetime,lon||','||lat as lonlat ,lon,lat";
    sql += ",tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en ";
    sql +=",mileage,modem_id,status as input_status"
...
function start(date_gen_report) 
{
    //AND modem_id='1010005002' 
      ...
      //  var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE  modem_id like '%14218%'   ORDER BY db_name";
        ...
        //WHERE db_name='db_10034' WHERE db_name='db_10034' WHERE db_name='db_10034'  AND modem_id='143190871464'  AND modem_id='143190871464'  
        var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10039'  ORDER BY db_name DESC";
...
//after 143190871432  '2020-10-08'/10 /19 /21 /22 open_rownumber = xres.rownum; //- 1;
//TypeError: Cannot read property 'rownum' of undefined
//start('2020-11-14');
...
schedule.scheduleJob(rule, function ()
{
   ...
   // end_date = date_now._i + ' 06:59'                             //'2017-02-01 06:59:00'
    ...
    console.log('Nissan find total working  This runs at 00:25 every day. '+start_date);
    debugger;
...
function start_manual(date_gen_report,callback) 
{
    //AND modem_id='1010005002' 
      ...
      //  var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE  modem_id like '%14218%'   ORDER BY db_name";
        //WHERE modem_id='142181155695' 
        //WHERE db_name='db_10034' WHERE db_name='db_10034' WHERE db_name='db_10034'  AND modem_id='143190871421'  AND modem_id='143190871421'
        var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle WHERE db_name='db_10039'    ORDER BY db_name DESC";
       // console.log('start genreport ' + date_gen_report + ' timenow : ' + irp.timenow()); AND modem_id='143190871314' AND modem_id='143190871314'
        ...
        db.get_rows(ipm, sql, function (res) 
        {
            if (res.length > 0) 
            {
                async.eachSeries(res, function (row, next) 
                {
                     //  debugger;
                    // console.log(row);
                    console.log(row.modem_id,date_gen_report);
                    var para = { 'id': 1, 'db_name': row.db_name, 'modem_id': row.modem_id, 'start_time': date_gen_report + ' 00:00', 'end_time': date_gen_report + ' 23:59', 'date_gen_report': date_gen_report, 'message': '' }
...
var datetimer_start=11;
function doit()
{
    datetimer_start =datetimer_start <= 9 ? "0"+datetimer_start  : datetimer_start;
    var start_date = '2021-10-'+datetimer_start;
 //   var start_date = '2021-05-'+datetimer_start;
    start_manual(start_date,function(is_fin)
    {
        if(is_fin)
        {
            if(datetimer_start = 11)
            {
...
doit();
//start_manual('2021-10-09',function(is_fin){})