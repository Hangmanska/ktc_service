...
/*
    ...
    SELECT idate(gps_datetime::TIMESTAMP) as gps_datetime,lon||','||lat as lonlat,speed,mileage,tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en 
FROM ht_142181155752
   WHERE gps_datetime::TIMESTAMP >='2019-05-01 00:00' AND gps_datetime::TIMESTAMP <='2019-05-03 23:59'
  AND speed > '50'
 AND status = '3' AND satelites::int > 0 ORDER BY gps_datetime  
*/
...
function get_data(para,callback) 
{
    ...
    {
        var sql = ' ';
        sql += " SELECT idate(gps_datetime::TIMESTAMP) as gps_datetime,lon||','||lat as lonlat,speed,mileage,tambol||':'||amphur||':'||province as locations_th,etambol||':'||eamphur||':'||eprovince as locations_en FROM ht_" + para.modem_id + " ";
        sql += " WHERE gps_datetime::TIMESTAMP >=" + utl.sqote(para.start_time) + " AND gps_datetime::TIMESTAMP <=" + utl.sqote(para.end_time) + " ";
        sql += " AND speed > " + utl.sqote(para.speed_max) + " AND status = '3' AND satelites::int > 0 ORDER BY gps_datetime  "
...
//WHERE db_name='db_10034' 
    //var date_gen_report = '2016-06-25'; WHERE modem_id like '%14218%'
    var sql = "SELECT modem_id,db_name,speedmax,track_every  FROM master_config_vehicle  ";
    sql += "  WHERE db_name <= max_dbname() AND to_char(time_inserver, 'YYYY-MM-DD')=to_char(now(), 'YYYY-MM-DD')  ";
    sql += " AND db_name !='db_10036' ORDER BY db_name ASC ";
