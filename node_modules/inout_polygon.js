...
var irec_now = require('htt_master_recnow.js');
...
function is_infactory(blackbox_id_truck, htt_place, lon, lat, callback)
{
    var result = [];
    ...
    db.get_rows(pg_query, sql, function (rows) {
              ...
              //#region 'ลานนอก'
                case 'ลานนอก': {
                    ...
                    if (htt_place != 'PARK_OUTSIDE')
                    {
                        ...
                        var sql_update = "UPDATE rec_now SET htt_place=" + utl.sqote(place_en) + ",htt_factorytime=" + utl.sqote(create_time()) + " WHERE blackbox_id=" + utl.sqote(blackbox_id_truck);

                        dtc2cane.iexcute(pg_realtime, sql_update, function (res_update) {
                            console.log(res_update);
                        });

                        // ikey_recnow.val = place_en;
                        //   irec_now.set_recnow_operate(ikey_recnow, function (x) { console.log('set factory ' + place_en + ' ' + x); })
                ...
                case 'ลานใน':
                    ...
                    if (htt_place != 'PARK_INSIDE')
                    {
                        ...
                        irec_now.get_transectionid(blackbox_id_truck, function (res_transectionid)
                        {
                            if (res_transectionid != 0)
                            {
                                var cane_para_timestap = res_transectionid + ',' + utcp.now() + ',park_inside_timestamp'
                                dtc2cane.set_delivery_vehicle_timestamp(cane_para_timestap, function (res_timestap)
                                {
                                    if (res_timestap != ' ')
                                    {
        ...
        else
        {
            ...
            if (htt_place == 'PARK_INSIDE')
            {
                ...
                var sql_clr = "UPDATE rec_now SET htt_place='ROAD_WITHOUT_CANE',htt_match_harvester_truck=' ',htt_truck_distance='0', htt_is_send_order_distance='0',htt_status_truck='EMPTY'  WHERE blackbox_id=" + utl.sqote(blackbox_id_truck);
                ...
                irec_now.get_transectionid(blackbox_id_truck, function (res_transectionid)
                {
                    if (res_transectionid != 0)
                    {
                        var cane_para_timestap = res_transectionid + ',' + utcp.now() + ',factory_leaving_timestamp'
                        dtc2cane.set_delivery_vehicle_timestamp(cane_para_timestap, function (res_timestap) {
                            if (res_timestap != ' ')
                            {
                                console.log(cane_para_timestap + ' set_delivery_vehicle_timestamp ' + res_timestap);
...
setTimeout(function () {
    ...
    //is_infactory('189600118236','park_inside', '102.095436', '16.107973', function (x) {
    //    debugger;
    //    console.log(x);
    //})
